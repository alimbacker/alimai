...
export default function Admin({ onLogout }) {
  ...
  function exportCsv() {
    ...
    const rows = users.map((u) => [
      u.email, u.name || "", u.status || "active", u.is_admin ? "yes" : "no",
      u.created_at || "", u.last_login || "never",
    ]);
    ...
    link.download = "alim-users.csv";
    link.click();
  }

  async function changeStatus(u, status) {
    try { await api.setUserStatus(u.id, status); setBanner(`${u.email} → ${status}`); loadAll(); }
    catch (e) { setError(e.message); }
  }
  async function resetPw(u) {
    try { const r = await api.resetUserPassword(u.id); setBanner(`Temp password for ${u.email}: ${r.tempPassword}`); }
    catch (e) { setError(e.message); }
  }
      ...
      {r && (
        <div className="panel" style={{ marginTop: 16 }}>
          <div className="panel-title">
            Retrieval Analytics <span className="muted-inline">({r.turns} turns · 30d)</span>
          </div>
          ...
          <div className="chart-row" style={{ marginTop: 16 }}>
            <div className="panel sub">
              <div className="panel-title">Clarification trend</div>
              <BarChart data={analytics.clarificationTrend} color="var(--amber)" />
            </div>
            <div className="panel sub">
              <div className="panel-title">Avg latency (ms)</div>
              <BarChart data={analytics.latencyTrend} color="#5aa9e6" suffix=" ms" />
            </div>
          </div>
        </div>
      )}
      ...
      <div className="panel" style={{ marginTop: 16 }}>
        ...
        {admins.map((a) => (
          <div className="admin-row" key={a.id}>
            ...
            {!a.bootstrap && (
              <button className="link-danger" onClick={() => demote(a)}>Remove admin</button>
            )}
          </div>
        ))}
